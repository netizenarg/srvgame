...
bool LootTableManager::LoadLootTables(const std::string& filePath) {
    if (!filePath.empty()) file_path_ = filePath;
    std::ifstream file(file_path_);
    if (!file.is_open()) {
        Logger::Error("LootTableManager: Failed to open file '{}'", file_path_);
        return false;
    }
    try {
...
bool LootTableManager::SaveLootTables(const std::string& filePath) const {
    std::string path = filePath.empty() ? file_path_ : filePath;
    std::ofstream file(path);
    if (!file.is_open()) {
        Logger::Error("LootTableManager: Failed to create file '{}'", path);
        return false;
    }
    try {
        ...
        file.close();
        Logger::Info("LootTableManager: Saved {} tables to '{}'", GetTableCount(), path);
        return true;
    } catch (const std::exception& e) {
