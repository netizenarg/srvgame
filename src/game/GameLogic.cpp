...
std::vector<uint64_t> GameLogic::GetSessionsInRadius(glm::vec3 position) {
    if (getSessionIdsInRadiusCb_)
        return getSessionIdsInRadiusCb_(position, ConfigManager::GetInstance().GetFloat("world.interest_radius", 100.0f));
    return {};
}

void GameLogic::BroadcastRemotePlayerSpawn(const PlayerSpawnData& data) {
    auto sessionIds = GetSessionsInRadius(data.position);
    for (auto sid : sessionIds) {
        if (sendPlayerSpawnCb_)
            sendPlayerSpawnCb_(sid, data);
    }
...
void GameLogic::BroadcastPlayerDespawn(uint64_t session_id, uint64_t player_id) {
    ...
    data.timestamp = GetCurrentTimestamp();
    data.player_id = player_id;
    auto sessionIds = GetSessionsInRadius(player->GetPosition());
    for (auto sid : sessionIds) {
        if (sid != session_id) {
            if (sendPlayerDespawnCb_)
                sendPlayerDespawnCb_(sid, data);
        }
    }
}
...
void GameLogic::SendAuthentication(uint64_t session_id, const std::string& message, uint64_t player_id) {
    if (sendAuthResponseCb_)
        sendAuthResponseCb_(session_id, message, player_id);
    else
        Logger::Error("No sendAuthResponseCb_ set");
}
...
void GameLogic::SetSendReplyCallback(std::function<void(uint64_t, const std::vector<uint8_t>&)> cb) {
    sendReplyCb_ = std::move(cb);
}

void GameLogic::SetGetSessionIdsInRadiusCallback(std::function<std::vector<uint64_t>(const glm::vec3&, float)> cb) {
    getSessionIdsInRadiusCb_ = std::move(cb);
}
...
void GameLogic::OnAuthentication(const AuthenticationData& data) {
    ...
    if (pm.PlayerExists(data.username)) {
        ...
        if (player) {
            ...
            if (data.password.empty()) message += " (Warning: no password set)";
        } else {
...
void GameLogic::SpawnerLoop() {
    Logger::Info("GameLogic::SpawnerLoop started");
    LogicCore::SpawnerLoop();
    Logger::Info("GameLogic::SpawnerLoop stopped");
}
