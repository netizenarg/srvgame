...
bool SkillSystem::LoadSkillData(const std::string& file_path) {
    ...
    try {
        ...
        if (!file.is_open()) {
            Logger::Error("Failed to open skill data file: {}", file_path);
            return false;
        }
        ...
        if (!json_data.is_array()) {
            Logger::Error("Invalid skill data format");
            return false;
        }
        ...
        Logger::Info("Loaded {} skills from {}", skill_database_.size(), file_path);
        return true;
    } catch (const std::exception& e) {
        Logger::Error("Error loading skill data: {}", e.what());
        return false;
    }
}
...
bool SkillSystem::SaveSkillData(const std::string& file_path) {
    ...
    try {
        ...
        if (!file.is_open()) {
            Logger::Error("Failed to open skill data file for writing: {}", file_path);
            return false;
        }
        ...
        Logger::Error("Error saving skill data: {}", e.what());
        return false;
    }
}
...
bool SkillSystem::LearnSkill(uint64_t player_id, const std::string& skill_id, int level) {
    ...
    if (skill_data_it == skill_database_.end()) {
        Logger::Warn("Skill not found: {}", skill_id);
        return false;
    }
    ...
    if (!MeetsSkillRequirements(player_id, skill_id)) {
        Logger::Warn("Player {} doesn't meet requirements for skill: {}", player_id, skill_id);
        return false;
    }

    // Check if player has skill points
    if (!HasSkillPoint(player_id)) {
        Logger::Warn("Player {} has no skill points", player_id);
        return false;
    }
    ...
    Logger::Info("Player {} learned skill: {} at level {}", player_id, skill_id, level);
...
bool SkillSystem::ForgetSkill(uint64_t player_id, const std::string& skill_id) {
    ...
    Logger::Info("Player {} forgot skill: {}", player_id, skill_id);
...
bool SkillSystem::UpgradeSkill(uint64_t player_id, const std::string& skill_id) {
    ...
    Logger::Info("Player {} upgraded skill: {} to level {}", player_id, skill_id, player_skill.level);
...
bool SkillSystem::UseSkill(uint64_t player_id, const std::string& skill_id, uint64_t target_id, const glm::vec3& target_position) {
    ...
    Logger::Info("Player {} used skill: {}", player_id, skill_id);
...
bool SkillSystem::InterruptSkill(uint64_t player_id, const std::string& skill_id) {
    ...
    if (player_data.channeling_skills.find(skill_id) == player_data.channeling_skills.end()) {
        return false;
    }
...
bool SkillSystem::UnlockSkillTree(uint64_t player_id, const std::string& tree_id) {
    ...
    if (player_data.unlocked_trees.find(tree_id) != player_data.unlocked_trees.end()) {
...
// Serialization – now always uses database, no file fallback
...
nlohmann::json SkillSystem::SerializePlayerSkills(uint64_t player_id) const {
    ...
    return (it != player_skills_.end()) ? it->second.Serialize() : nlohmann::json();
}
...
bool SkillSystem::DeserializePlayerSkills(uint64_t player_id, const nlohmann::json& data) {
    ...
    try {
        ...
        Logger::Error("Error deserializing player skills: {}", e.what());
        return false;
    }
}
...
void SkillSystem::ApplyAreaEffect(uint64_t caster_id, const glm::vec3& center, float radius, const std::string& skill_id) {
    ...
    Logger::Info("Applying area effect from skill {} at position {}, {}, {} ({}; {})",
                 skill_id, center.x, center.y, center.z, caster_id, radius);
}
...
void SkillSystem::ApplyConeEffect(uint64_t caster_id, const glm::vec3& direction, float angle, float range, const std::string& skill_id) {
    ...
    (void)direction;
    Logger::Info("Applying cone effect from skill {} ({}; {}; {})", skill_id, caster_id, angle, range);
}
...
void SkillSystem::ApplyDamageEffect(uint64_t caster_id, uint64_t target_id, const SkillEffect& effect) {
    ...
    Logger::Info("Applying damage effect: {} damage from {} to {}", damage, caster_id, target_id);
...
void SkillSystem::ApplyHealingEffect(uint64_t caster_id, uint64_t target_id, const SkillEffect& effect) {
    ...
    Logger::Info("Applying healing effect: {} healing from {} to {}", healing, caster_id, target_id);
...
void SkillSystem::ApplyBuffEffect(uint64_t caster_id, uint64_t target_id, const SkillEffect& effect) {
    Logger::Info("Applying buff effect: {} from {} to {}", effect.effect_type, caster_id, target_id);
}

// Database operations – all now use DbManager unconditionally
bool SkillSystem::LoadSkillDataFromDatabase() {
    try {
        nlohmann::json result = DbManager::GetInstance().Query("SELECT skill_data FROM skill_database");
        if (!result.is_array()) {
            Logger::Error("LoadSkillDataFromDatabase: result is not an array");
            return false;
        }
        for (const auto& row : result) {
            if (!row.contains("skill_data")) {
                Logger::Warn("LoadSkillDataFromDatabase: row missing skill_data");
                continue;
            }
            std::string skill_json_str = row["skill_data"];
            nlohmann::json skill_json = nlohmann::json::parse(skill_json_str);
            SkillData skill;
            skill.Deserialize(skill_json);
            skill_database_[skill.id] = skill;
        }
        Logger::Info("Loaded {} skills from database", skill_database_.size());
        return true;
    } catch (const std::exception& e) {
        Logger::Error("Error loading skill data from database: {}", e.what());
        return false;
    }
}
...
bool SkillSystem::SaveSkillDataToDatabase() {
    try {
        ...
        for (const auto& [id, skill] : skill_database_) {
            nlohmann::json skill_json = skill.Serialize();
            std::string skill_str = skill_json.dump();
            std::string escaped = db.EscapeString(skill_str);
            std::string query = "INSERT INTO skill_database (skill_id, skill_data) VALUES ('" +
                                id + "', '" + escaped +
                                "') ON CONFLICT (skill_id) DO UPDATE SET skill_data = EXCLUDED.skill_data";
            db.Query(query);
        }
        Logger::Info("Saved {} skills to database", skill_database_.size());
        return true;
    } catch (const std::exception& e) {
        Logger::Error("Error saving skill data to database: {}", e.what());
        return false;
    }
}
...
bool SkillSystem::LoadPlayerSkillsFromDatabase(uint64_t player_id) {
    try {
        std::string query = "SELECT skill_data FROM player_skills WHERE player_id = " + std::to_string(player_id);
        nlohmann::json result = DbManager::GetInstance().Query(query);
        if (result.is_array() && result.empty()) {
            // No existing data – player has no skills yet (not an error)
            return true;
        }
        if (!result.is_array() || result.size() != 1 || !result[0].contains("skill_data")) {
            Logger::Error("LoadPlayerSkillsFromDatabase: unexpected result format");
            return false;
        }
        std::string skill_data_str = result[0]["skill_data"];
        nlohmann::json json_data = nlohmann::json::parse(skill_data_str);
        PlayerSkillData player_data;
        player_data.Deserialize(json_data);
        player_skills_[player_id] = player_data;
        Logger::Info("Loaded skills for player {} from database", player_id);
        return true;
    } catch (const std::exception& e) {
        Logger::Error("Error loading player skills from database: {}", e.what());
        return false;
    }
}
...
bool SkillSystem::SavePlayerSkillsToDatabase(uint64_t player_id) {
    ...
    try {
        nlohmann::json json_data = it->second.Serialize();
        std::string json_str = json_data.dump();
        std::string escaped = DbManager::GetInstance().EscapeString(json_str);
        std::string query = "INSERT INTO player_skills (player_id, skill_data) VALUES (" +
                            std::to_string(player_id) + ", '" + escaped +
                            "') ON CONFLICT (player_id) DO UPDATE SET skill_data = EXCLUDED.skill_data";
        DbManager::GetInstance().Query(query);
        Logger::Info("Saved skills for player {} to database", player_id);
        return true;
    } catch (const std::exception& e) {
        Logger::Error("Error saving player skills to database: {}", e.what());
        return false;
    }
}
